...
fn test() {
    ...
    let test = Test::from_iter_with_yaml(yaml_str, args.iter()).unwrap();
    assert_eq!(test.a0, 0);
...
fn test_args_with_other_attributes() {
    ...
    bar: "one"
    "#;
    let test = Bar::from_args_with_yaml(yaml_str);
    match dbg!(test) {
        Err(_) => assert!(false),
        _ => assert!(true),
    }
...
fn test_yaml_failed() {
    ...
    "#;
    let args = vec!["test"];
    let test = Test::from_iter_with_yaml(yaml_str, args.iter());
    match test {
        Err(_) => assert!(true),
        _ => assert!(false),
    }
...
fn test_yaml_args() {
    let yaml_str = r#"
        ...
        d3: [133]
    "#;
    let test = Test::from_args_with_yaml(yaml_str).unwrap();
    assert_eq!(test.a0, 0);
    assert_eq!(test.a1, 1);
    assert_eq!(test.a2, 102);
...
fn test_json_args() {
    ...
    "#;
    let test = Test::from_args_with_yaml(json_str).unwrap();
    assert_eq!(test.a0, 0);
    assert_eq!(test.a1, 1);
    assert_eq!(test.a2, 102);
    assert_eq!(test.a3, 103);
